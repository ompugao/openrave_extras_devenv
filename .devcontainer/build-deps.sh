...
docker build -t ompugao/ros:melodic-desktop-full-${suffix}-gl -f Dockerfile.ros --build-arg from=$baseimage --build-arg cuda_major=$cuda_major --build-arg cuda_minor=$cuda_minor --progress=plain .
docker build -t ompugao/openrave-deps:melodic-desktop-full-${suffix}-gl -f Dockerfile.openrave-deps --target dev --build-arg from=ompugao/ros:melodic-desktop-full-${suffix}-gl --progress=plain . 
